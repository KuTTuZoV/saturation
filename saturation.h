...
extern "C" {
    SATURATION_EXPORT QByteArray createPicture(unsigned char * imageData, int fileSize, int imageWidth, int imageHeight);
    SATURATION_EXPORT float Min( float fR, float fG, float fB );
    SATURATION_EXPORT float Max( float fR, float fG, float fB);
    SATURATION_EXPORT void HLSToRGB( unsigned char &R, unsigned char &G, unsigned char &B, double H, double S, double L);
    SATURATION_EXPORT void RGBToHSL( int R, int G, int B, double& H, double& S, double& L);
    SATURATION_EXPORT void filter(unsigned char * image, unsigned char * result,  int width, int height, float bright);
}
